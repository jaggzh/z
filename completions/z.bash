...
--clipboard --cb
--image --img
--image-ext --img-ext --iext
--image-alias --img-alias --ialias
--audio
--audio-ext --au-ext --aext
--audio-alias --au-alias --aalias
--media
...
## Session folder handling
  __complete_sessions() {
    local token="$1"
    local prefix="" needle="$token"
  
    # If someone ever uses --session=foo style, preserve the "opt=" prefix.
    if [[ "$needle" == *=* ]]; then
      prefix="${needle%%=*}="
      needle="${needle#*=}"
    fi
  
    local base="$dir_sessions_user"
    [[ -d "$base" ]] || return 0
  
    # Normalize: strip any leading "./"
    needle="${needle#./}"
  
    local dirpart="" filepart="$needle"
    local want_list_children=0
  
    if [[ "$needle" == */ ]]; then
      want_list_children=1
      dirpart="${needle%/}"
      filepart=""
    elif [[ "$needle" == */* ]]; then
      dirpart="${needle%/*}"
    ...
    local target="$base"
    [[ -n "$dirpart" ]] && target="$base/$dirpart"
  
    [[ -d "$target" ]] || return 0
  
    local -a matches=()
    local child name cand
  
    # List immediate subdirectories of $target
    while IFS= read -r -d '' child; do
      name="${child##*/}"
  
      # Filter by typed prefix (filepart) unless we’re listing children after a trailing slash
      if [[ -z "$filepart" || "$name" == "$filepart"* ]]; then
        if [[ -n "$dirpart" ]]; then
          cand="$dirpart/$name"
        else
          cand="$name"
    ...
    # Special behavior:
    # If the user typed an exact dir name (no slash) and it’s the only match,
    # then return it with a trailing "/" so the next <tab> lists its children.
    if [[ "$want_list_children" -eq 0 && "$needle" != */* && ${#matches[@]} -eq 1 && "${matches[0]}" == "$needle" ]]; then
      local exact="$base/$needle"
      if [[ -d "$exact" ]] && find "$exact" -mindepth 1 -maxdepth 1 -type d -print -quit 2>/dev/null | grep -q .; then
        COMPREPLY=("${prefix}${needle}/")
        return 0
      fi
    fi
